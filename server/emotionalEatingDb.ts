...
export async function logEmotionalEating(data: {
  ...
  const [log] = await db.insert(emotionalEatingLogs).values(data);
  return log;
}
...
export async function addMedication(data: {
  ...
  const [medication] = await db.insert(medications).values(data as any);
  return medication;
}
...
export async function logMedicationTaken(data: {
  ...
  const [log] = await db.insert(medicationLogs).values(data);
  return log;
}
